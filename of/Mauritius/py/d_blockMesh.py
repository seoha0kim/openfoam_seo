...
for i in range(np.shape(of_block)[0]):
    ...
    yi = np.abs(of_xyz[1][ix] - of_xyz[1][ix1])
    dx = (xi**2 + yi**2)**(1/2)
    of_br[i,[0,1]] = np.maximum(dx/dx_min*n_amp,[1,1])
    n_g0[i] = np.log(np.max(of_br[i,[0,1]])*(r_g-1)+1) / np.log(r_g)
    def er(ri):
        return (np.log(dx/(dx_min/n_amp)*(ri-1)+1)/np.log(ri) - n_g0[i])
    ...
    yi = np.abs(of_xyz[1][ix] - of_xyz[1][ix1])
    dx = (xi**2 + yi**2)**(1/2)
    of_br[i,[2,3]] = np.maximum(dx/dx_min*n_amp,[1,1])
    n_g1[i] = np.log(np.max(of_br[i,[2,3]])*(r_g-1)+1) / np.log(r_g)
    def er(ri):
        return (np.log(dx/(dx_min/n_amp)*(ri-1)+1)/np.log(ri) - n_g1[i])
    r_0 = optimize.leastsq(er, [r_g,r_g])
    R_g0[i,[2,3]] = r_0[0]**(n_g0[i]-1)
    # f_bl.write('\nBock ID: %d, # of mesh cell in a block, %d, r: %s'%( \
    f_bl.write(', %s'%( \
        # i, n_g0[i], ' '.join('%f'%r_0[0][j] for j in range(2))
        ' '.join('%f'%R_g0[i,j+2] for j in range(2))
        ))

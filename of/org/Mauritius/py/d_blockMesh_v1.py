...
for i in range(np.shape(of_block)[0]):
    ...
    # of_br[i,] = np.maximum(np.r_[dx[0],dx[1],1]/dx_min*n_amp,[1,1,1,1,1])
    # n_bl = dx/dx_min*n_amp
    # print(np.shape(n_bl))
    # print(n_bl)
    # f_bl.write('\n# of mesh cell in a block, %d: %s'%(i, ' '.join(str(int(n_bl[j])) for j in range(np.shape(n_bl)[0]))))
    # of_br[i,[0,1]] = dx/dx_min*n_amp
    # f_bl.write('\n# of mesh cell in a block, %d: %s'%(i, ' '.join(str(int(of_br[i,j])) for j in range(2))))
    of_br[i,0] = int(np.max(np.r_[dx/dx_min*n_amp,1]))
    # f_bl.write('\n# of mesh cell in a block, %d: %d'%(i, of_br[i,0]))
    n_g0[i] = np.log(of_br[i,0]*(r_g-1)+1) / np.log(r_g)
    def er(ri):
        # return (np.log(l/a*(ri-1)+1)/np.log(ri) - ni)**2
        return (np.log(dx[0]/(dx_min/n_amp)*(ri-1)+1)/np.log(ri) - n_g0[i])
    # r_0 = optimize.leastsq(er, [r_g,r_g])
    r_0 = optimize.leastsq(er, r_g)
    R_g0[i,0] = r_0[0]**(n_g0[i]-1)
    def er(ri):
        return (np.log(dx[1]/(dx_min/n_amp)*(ri-1)+1)/np.log(ri) - n_g0[i])
    r_0 = optimize.leastsq(er, r_g)
    # f_bl.write('\nBlock ID: %d, r1 = %f, r2 = %f'%(i, r_0[0][0], r_0[0][1]))
    R_g0[i,1] = r_0[0]**(n_g0[i]-1)
    ...
    yi = np.abs(of_xyz[1][ix] - of_xyz[1][ix1])
    dx = (xi**2 + yi**2)**(1/2)
    of_br[i,2] = int(np.max(np.r_[dx/dx_min*n_amp,1]))
    n_g1[i] = np.log(of_br[i,2]*(r_g-1)+1) / np.log(r_g)
    def er(ri):
        return (np.log(dx[0]/(dx_min/n_amp)*(ri-1)+1)/np.log(ri) - n_g1[i])
    r_0 = optimize.leastsq(er, r_g)
    R_g0[i,2] = r_0[0]**(n_g0[i]-1)
    def er(ri):
        return (np.log(dx[1]/(dx_min/n_amp)*(ri-1)+1)/np.log(ri) - n_g1[i])
    r_0 = optimize.leastsq(er, r_g)
    R_g0[i,3] = r_0[0]**(n_g0[i]-1)

    f_bl.write('\nBlock ID: %d, R1 = %f, R2 = %f, R3 = %f, R4 = %f'%(i, R_g0[i][0], R_g0[i][1], R_g0[i][2], R_g0[i][3]))
        ...
        for ii in range(np.shape(of_block)[0]):
            if (ii == 0):
                ...
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[0], n_g1[17], 1/R_g0[ii,0], 1/R_g0[ii,1], 1/R_g0[ii,1], 1/R_g0[ii,0], 1/R_g0[ii,2], 1/R_g0[ii,3], 1/R_g0[ii,3], 1/R_g0[ii,2] ) )
            elif (ii == 17):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    # (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[ii], n_g1[ii], R_g0[ii], 1/R_g1[ii] ) )
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[17], n_g1[17], R_g0[ii,0], R_g0[ii,1], R_g0[ii,1], R_g0[ii,0], 1/R_g0[ii,2], 1/R_g0[ii,3], 1/R_g0[ii,3], 1/R_g0[ii,2] ) )
            elif (ii == 74):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[0], n_g1[79], 1/R_g0[ii,0], 1/R_g0[ii,1], 1/R_g0[ii,1], 1/R_g0[ii,0], R_g0[ii,2], R_g0[ii,3], R_g0[ii,3], R_g0[ii,2] ) )
            elif (ii == 79):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[17], n_g1[79], R_g0[ii,0], R_g0[ii,1], R_g0[ii,1], R_g0[ii,0], R_g0[ii,2], R_g0[ii,3], R_g0[ii,3], R_g0[ii,2] ) )
            elif ((ii > 0) and (ii < 17)):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), of_br[ii,0], n_g1[17], 1, 1, 1, 1, 1/R_g0[ii,2], 1/R_g0[ii,3], 1/R_g0[ii,3], 1/R_g0[ii,2] ) )
            elif ((ii > 74) and (ii < 79)):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), of_br[gam_ii_jj[ii],0], n_g1[79], 1, 1, 1, 1, R_g0[ii,2], R_g0[ii,3], R_g0[ii,3], R_g0[ii,2] ) )
            elif (ii == 18) or (ii == 36) or (ii == 50) or (ii == 64) or (ii == 68):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[0], of_br[gam_ii_jj[ii],2], 1/R_g0[ii,0], 1/R_g0[ii,1], 1/R_g0[ii,1], 1/R_g0[ii,0], 1, 1, 1, 1 ) )
            elif (ii == 35) or (ii == 49) or (ii == 63) or (ii == 67) or (ii == 73):
                # f.write('    hex (%s) (%03d %03d 1) simpleGrading (%7.3f %7.3f 1)\n'% \
                f.write('    hex (%s) (%03d %03d 1) edgeGrading (%.1e %.1e %.1e %.1e %.1e %.1e %.1e %.1e 1 1 1 1)\n'% \
                    (' '.join( '%03d'%of_block[ii][jj] for jj in range(8) ), n_g0[17], of_br[ii,2], R_g0[ii,0], R_g0[ii,1], R_g0[ii,1], R_g0[ii,0], 1, 1, 1, 1 ) )

...
for id_angle in range(1):
    ...
    # #     raise e
    try:
        os.chdir('%s/postProcessing/forceCoeffs/0'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
        ...
        os.chdir('%s/postProcessing/forces/0'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
    ...
    try:
        os.chdir('%s/postProcessing/forceCoeffs/50'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
    ...
    # except Exception as e:
        ...
        os.chdir('%s/postProcessing/forces/50'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
    ...
    try:
        os.chdir('%s/postProcessing/forceCoeffs/500'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
        ...
        os.chdir('%s/postProcessing/forces/500'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
    ...
    try:
        os.chdir('%s/postProcessing/forceCoeffs/1000'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
        ...
        os.chdir('%s/postProcessing/forces/5000'%s_angle)
        p = os.popen('pwd',"r")
        while 1:
            line = p.readline()
            if not line: break
...
fig1 = plt.figure()
plt.subplot(121)
plt.plot(xn[:,0],xn[:,2]*Ur**2/U**2*0.1/B,color=[0,0,1])
plt.plot(xn[:,0],xn[:,3]*Ur**2/U**2*0.1/B,color=[0,.25,0])
plt.plot(xn[:,0],xn[:,1]*Ur**2/U**2*0.1/B**2,color=[1,0,0])
plt.plot(xn[:,0],xn[:,4]*Ur**2/U**2*0.1/B,color=[0,1,0])
plt.plot(xn[:,0],xn[:,5]*Ur**2/U**2*0.1/B,color=[0,.75,0])
plt.plot(xn[:,0],(xn[:,4]+xn[:,5])*Ur**2/U**2*0.1/B,color=[0,.5,0])
plt.subplot(122)
id_123 = 3-1
...
plt.subplot(121)
plt.plot(yn[:,0],(yn[:,1]+yn[:,4]+yn[:,7])/(1/2*1.225*U**2*B),color=([0,0,.5]),linewidth=2)
plt.plot(yn[:,0],(yn[:,2]+yn[:,5]+yn[:,8])/(1/2*1.225*U**2*B),color=([0,.5,0]),linewidth=2)

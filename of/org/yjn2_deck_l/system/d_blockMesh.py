...
of_xyz = [
    ...
    np.r_[np.ones(of_nv)*-0.5,np.ones(of_nv)], \
    ]
# print(of_xyz)
...
for i in range(np.shape(of_block)[0]):
    ...
    ix1 = of_block[i][[0,1]]
    yi = np.abs(of_xyz[1][ix] - of_xyz[1][ix1])
    of_br[i,] = np.maximum(np.r_[xi,yi,1]/dx_min/1,[1,1,1,1,1])
...
I_u = np.array([1.,1.,1.])*1e-2*5
f_in.write('\nTurbulence intensity: %s'%' '.join([ str(x) for x in I_u ]))
sig_U = U_mu * I_u
f_in.write('\nsig_U: %s'%' '.join([ str(x) for x in sig_U ]))

k = 1/2*sum(sig_U**2)
f_in.write('\nk: %f'%k)

epsilon = [C_mu**0.75 * k**1.5 / (x * 0.1) for x in BD]
f_in.write('\nepsilon: ' + ' '.join('%f'%x for x in epsilon))

d_t = dx_min/U_mu
f_in.write('\nd_t: %f'%d_t)

T_f = [x / U_mu for x in dx]
f_in.write('\nT_f: ' + ' '.join('%f'%x for x in T_f))

...
C.DLM(1,1+3) = mphint2(model, ...
    sprintf('(%s.nxmesh*p + %s.rho*%s.u_tau*%s.u_tangx/%s.uPlus) / (1/2*%s.rho*(seo_U_in^2))' ...
        ,s_spf,s_spf,s_spf,s_spf,s_spf,s_spf), ...
    'line','selection','box5','dataset',s_dset)/sb.D;
C.DLM(1,2+3) = mphint2(model, ...
    sprintf('(%s.nymesh*p + %s.rho*%s.u_tau*%s.u_tangy/%s.uPlus) / (1/2*%s.rho*(seo_U_in^2))' ...
        ,s_spf,s_spf,s_spf,s_spf,s_spf,s_spf), ...
    'line','selection','box5','dataset',s_dset)/sb.D;
C.DLM(1,3+3) = mphint2(model, ...
    sprintf('((%s.nxmesh*p + %s.rho*%s.u_tau*%s.u_tangx/%s.uPlus)*y + (%s.nymesh*p + %s.rho*%s.u_tau*%s.u_tangy/%s.uPlus)*-x)/(1/2*%s.rho*(seo_U_in^2))' ...
